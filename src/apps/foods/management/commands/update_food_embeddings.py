...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total == 0:
            self.stdout.write(self.style.SUCCESS("Todos os alimentos já possuem embeddings."))
            return

        self.stdout.write(f"Iniciando processamento de {total} alimentos com concorrência...")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_food = {executor.submit(process_food, fid): fid for fid in food_ids}
            
            for i, future in enumerate(concurrent.futures.as_completed(future_to_food), 1):
                ...
                if i % 20 == 0 or i == total:
                    self.stdout.write(f"Progresso: {i}/{total} (Sucessos: {success_count}, Erros: {error_count})")

        self.stdout.write(self.style.SUCCESS(f"Concluído! {success_count} alimentos atualizados, {error_count} erros."))
