...
### Registrar uma Refeição Simples

**Endpoint:** `POST /api/foods/tracker/log/`

**Autenticação:** Necessária.

**Descrição:** Registra um único item alimentar como uma refeição (registro simples). Este endpoint pertence ao app Foods mas cria um registro `MealLog`.

**Corpo da Requisição:**

| Campo | Tipo | Obrigatório | Descrição |
|-------|------|-------------|-----------|
| `food` | integer | Sim | ID do alimento. |
| `quantity_g` | decimal | Sim | Quantidade em gramas (mínimo 0.1). |
...
```json
{
  ...
  "meal_log": {
    ...
    "food_name": "Banana",
    "quantity_g": 150.0,
    "consumed_at": "2026-04-11T11:35:00Z"
  },
  "warnings": []
}
```
...
**Erros:**
- `400 Bad Request` – Erros de validação (alimento não encontrado, quantidade inválida).
